...
<li><strong>写入增量命令</strong>：父进程将重写期间的命令写入AOF缓冲区和新AOF文件。</li>
</ol>
<pre><code>• **性能瓶颈与优化**：  
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div>• <strong>应急处理</strong>：</li>
</ol>
<pre><code>• **临时扩容**：调整`maxmemory`值并重启（需主从切换）。  
...
• <strong>手动介入场景</strong>：</li>
</ol>
<pre><code>• **多节点宕机**：需人工确认数据一致性后恢复。  
...
<li><strong>客户端限流</strong>：限制单个服务实例的请求并发数。</li>
</ol>
<pre><code>• **后续优化**：  
...
<li><strong>数据验证</strong>：通过压测工具（如JMeter）验证方案可行性。</li>
</ol>
<pre><code>• **高频考点延伸**：  
...
<p>• <strong>生产意识培养</strong>：<br>
• <strong>监控告警</strong>：任何方案必须配套监控（如大Key检测、慢查询分析）。<br>
• <strong>灰度发布</strong>：缓存策略变更时，先切流1%流量验证。</p>
