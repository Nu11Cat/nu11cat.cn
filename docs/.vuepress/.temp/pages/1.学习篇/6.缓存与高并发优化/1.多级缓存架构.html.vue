...
<li>异步批量或延迟写入数据库。<br>
• <strong>风险与优化</strong>：</li>
</ol>
<pre><code>• **数据丢失风险**：缓存宕机导致未持久化数据丢失，需结合WAL（Write-Ahead Logging）。  
• **批量合并写入**：将多次更新合并为一次数据库操作，减少IO压力。  
</code></pre>
<p>• <strong>应用场景</strong>：写密集且容忍最终一致性的场景（如点赞计数）。</p>
<hr>
...
<code v-pre>java       // 大V发帖时推送到粉丝的Feed列表       followers.forEach(follower -&gt;           redisTemplate.opsForList().leftPush(&quot;feed:&quot; + follower, post.toJSON())       );       // 控制列表长度，保留最新1000条       redisTemplate.opsForList().trim(&quot;feed:&quot; + follower, 0, 999);       </code>
• <strong>拉模式（读扩散）</strong>：<br>
• <strong>场景</strong>：普通用户读取Feed时，实时拉取关注用户的动态并合并。<br>
• <strong>缓存优化</strong></p>
...
<pre><code>• **锁续期（Redisson Watchdog）**：  
...
<li><strong>持久层优化</strong>：MySQL分库分表 + 读写分离，异步批量写入。</li>
</ol>
<pre><code>• **核心策略**：  
...
◦ <strong>分库分表</strong>：订单表按用户ID哈希分16库，每库256表。</li>
</ol>
<pre><code>• **容灾设计**：  
...
◦ 网关层限流（每秒10万请求），服务层熔断（失败率&gt;30%触发）。</li>
</ol>
<pre><code>• **效果**：  
...
<li><strong>异步更新</strong>：降级期间，后台线程异步刷新热点数据。</li>
</ol>
<pre><code>• **结果**：  
...
<li><strong>数据支撑</strong>：引用生产案例数据（如QPS提升比例）增强说服力。</li>
</ol>
<pre><code>• **高频考点**：  
...
• <strong>监控先行</strong>：没有监控的优化是盲目的，Prometheus + Grafana必备。</p>
