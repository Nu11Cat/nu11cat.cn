...
<li>Socket缓冲区 → 网卡（DMA拷贝）。<br>
<strong>总拷贝次数</strong>：4次，上下文切换：4次。</li>
</ol>
<p>• <strong>零拷贝实现（<code v-pre>FileChannel.transferTo()</code>）</strong>：</p>
<ol>
<li>磁盘数据 → 内核缓冲区（DMA拷贝）。</li>
<li>内核缓冲区 → 网卡（DMA拷贝）。<br>
<strong>总拷贝次数</strong>：2次，上下文切换：2次。</li>
</ol>
...
<li>查看完整TCP握手（SYN/SYN-ACK/ACK）及数据传输过程。<br>
• <strong>场景</strong>：定位连接超时、丢包导致的性能问题。</li>
</ol>
...
<ol start="2">
<li>Wireshark → <strong>Preferences → Protocols → TLS</strong>，添加密钥日志路径。<br>
• <strong>场景</strong>：调试Protobuf序列化异常或gRPC流控问题。</li>
</ol>
<hr>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div></li>
<li><strong>消费者</strong>：每个节点消费指定分区，通过WebSocket推送至本地连接。<br>
• <strong>优势</strong>：水平扩展能力（每增加一个节点，Kafka分区数同步扩容）。<br>
• <strong>分区策略示例</strong>：</li>
</ol>
