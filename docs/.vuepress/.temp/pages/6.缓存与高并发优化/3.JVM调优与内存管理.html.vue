...
• <strong>堆外缓存防护铁律</strong>：<br>
• <strong>资源释放</strong>：所有堆外缓存操作必须封装在<code v-pre>try-with-resources</code>中。<br>
• <strong>监控告警</strong>：通过Prometheus监控堆外内存使用率，超过80%触发告警。</li>
</ol>
...
• Mixed GC延迟降至50ms，商品详情页接口P99从200ms降至80ms。</p>
...
• <strong>效果验收标准</strong>：<br>
• <strong>GC停顿</strong>：ZGC/Shenandoah场景下，P99停顿 ≤ 5ms。<br>
• <strong>缓存命中率</strong>：L1缓存 ≥ 95%，L2缓存 ≥ 80%。<br>
• <strong>文档沉淀</strong>：每次调优后记录参数变更、效果数据、回滚方案。</li>
</ol>
...
• <strong>Netty未释放ByteBuf</strong>：通过<code v-pre>io.netty.util.ReferenceCountUtil.release(buffer)</code>手动释放。<br>
• <strong>JNI调用未回收</strong>：本地代码中<code v-pre>malloc</code>未调用<code v-pre>free</code>。</li>
</ol>
